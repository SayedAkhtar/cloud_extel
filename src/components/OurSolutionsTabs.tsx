...
const data = [
    {
        ...
        description: `<p>Think of small cells as the extra oxygen telecom operators need to handle growing data demands. These 5G-upgradable sites are the future-ready drivers of next-gen connectivity, transforming high-density areas with superior signal strength and sufficient capacity.</p></br><p> Our small cells are discreetly installed on streetlights and poles, seamlessly blending into the urban landscape while making a massive impact on network performance. </p>`,
        layout: 'start',
    },
    {
...
export default function OurSolutionsTabs() {
  ...
  return (
    <>
      {/* Navigation */}
      <section id="our-solutions-nav" className="pt-8 hidden md:block ">
       
          <nav className="flex space-x-8 border-b border-gray-300 w-full justify-between container">
            {data.map((item, index) => {
              return (
                <a
            ...
            >
              <span
                className={`h-[10px] w-[32px] border-[#233852] group-hover:border-white border-t-0 md:border-t border-b-0 md:border-b block `}
              ></span>
        ...
        <div className="w-full mx-auto py-[100px]">
          {data.map((item, index) => {
            return (
              <div key={_.uniqueId()}>
                {index !== 0 ? (
                  <hr className="my-[100px] border-transparent md:border-[#728090]" />
                ) : null}
                <CardTile
                  data-aos="fade-up"
...
const CardTile = forwardRef<HTMLDivElement,CardTileProps>(({image, title, description, layout, index }, ref) => {
    if(layout === 'end'){
        return (
          <div className="flex flex-col lg:flex-row-reverse items-start gap-8 mt-8 scroll-margin" ref={ref} data-index={index}
          >
            <div 
    ...
    return (
        ...
        <div className="flex flex-col lg:flex-row items-start justify-start gap-8 scroll-margin" ref={ref} id={"section-id-"+index} data-index={index}>
          <div 
          // data-aos="fade-right" data-aos-duration="500" 
          className="w-full lg:w-1/2">
            <img
